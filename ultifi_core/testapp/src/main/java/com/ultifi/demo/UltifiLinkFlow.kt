...
import com.ultifi.demo.ultifi.extractCode
    ...
    init {
        ...
        ultifiLink =
            UltifiLink.create(context, handler, object : UltifiLink.ServiceLifecycleListener {
                override fun onLifecycleChanged(link: UltifiLink?, isReady: Boolean) {
                    Log.d(TAG, "receiving link connection lifecycle event. IsReady: $isReady")
                    link?.let {
                        ultifiLinkConnectionFlow.tryEmit(UltifiLinkConnection(link, isReady))
                    }
                }
